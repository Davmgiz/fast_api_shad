...
async def creat_seller(async_client):
    """Создаёт продавца через API и возвращает seller_id"""
    seller_data = {
        "first_name": "John",
        "last_name": "Doe",
    ...
    assert seller_response.status_code == status.HTTP_201_CREATED, seller_response.text

    return seller_response.json()["id"]

# Тест на ручку создающую книгу
@pytest.mark.asyncio
async def test_create_book(async_client):

    seller_id = await creat_seller(async_client)
...
async def test_get_books(db_session, async_client):
    ...
    book = Book(author="Pushkin", title="Eugeny Onegin", year=2001, pages=104, seller_id=seller_id)
    book_2 = Book(author="Lermontov", title="Mziri", year=1997, pages=104, seller_id=seller_id)
...
async def test_delete_book(db_session, async_client):
    ...
    book = Book(author="Lermontov", title="Mtziri", pages=510, year=2024, seller_id=seller_id)
